...
bool Logger::checkDirectories(int numChannels) {
    vector<string> dirPaths = {AIPRO_PATH, ROOT_PATH, CONFIG_PATH, CHIMGS_PATH, CNT_PATH, CC_PATH, FD_PATH, LOG_PATH};
...
bool Logger::createLog() {
    ...
    // create a log file
    string newLogFilepath = string(LOG_PATH) + "/" + dayInfo + ".txt";
...
void Logger::writeData(Config &cfg, ODRecord &odRcd, FDRecord &fdRcd, CCRecord &ccRcd, Mat &frame,
    ...
    if ((preTm->tm_min == 29 && curTm->tm_min == 30) || (preTm->tm_min == 59 && curTm->tm_min == 0)) {
        ...
        if (preTm->tm_mday != curTm->tm_mday) {
            dayChanged = true;  // checked for all channels, but log is created just once(see createLog())
        ...
        if (cfg.ccChannels[vchID]) {
            ...
            if (ccEventLogs.size() > 0) {
                ...
                string txtPathCCEvent =
                    string(CC_PATH) + "/" + dayInfo + "/" + to_string(vchID) + "/txts/" + filenameCCEvent;
                ...
                string filenameCCEventImg =
                    std::format("{:02}{:02}{:02}.jpg", curTm->tm_hour, curTm->tm_min, curTm->tm_sec);
                string imgPathCCEventImg =
                    string(CC_PATH) + "/" + dayInfo + "/" + to_string(vchID) + "/imgs/" + filenameCCEventImg;
                imwrite(imgPathCCEventImg, frame, {cv::ImwriteFlags::IMWRITE_JPEG_QUALITY, 80});
            }
            ...
            /*if (ccLogs.size() > 0) {
                ...
                string filenameImg = std::format("{:02}{:02}{:02}.jpg", curTm->tm_hour, curTm->tm_min, curTm->tm_sec);
                string imgPathCC = string(CC_PATH) + "/" + dayInfo + "/" + to_string(vchID) + "/imgs/" + filenameImg;
...
void Logger::writeNowFD(int vchID, string dayInfo, int sec) {
    ...
    string txtPathFDNow = string(FD_PATH) + "/" + dayInfo + "/" + to_string(vchID) + "/now/" + filenameFDNow;
...
void Logger::writeNowCC(CCRecord &ccRcd, int vchID, string dayInfo, int sec) {
    ...
    string txtPathCCNow = string(CC_PATH) + "/" + dayInfo + "/" + to_string(vchID) + "/now/" + filenameCCNow;
